...
// IntLCS returns the longest common subsequence of two given int slices,
...
func IntLCS(A, B []int) (int, []int) {
	if len(A) == 0 || len(B) == 0 {
		return 0, []int{}
	}

	startIndex := findStartIndexInt(A, B)
	endIndexA, endIndexB := findEndIndicesInt(A, B, startIndex)
	...
	partialLCS := doLCSInt(A[startIndex:endIndexA], B[startIndex:endIndexB])

	lcs := make([]int, startIndex)
...
func findStartIndexInt(A, B []int) int {
	start, i, j := 0, 0, 0
...
func findEndIndicesInt(A, B []int, startIndex int) (int, int) {
...
func doLCSInt(A, B []int) []int {

	partialsLength := len(B) + 1
	partials := make([][]int, partialsLength)

	for _, elA := range A {
		newPartials := make([][]int, partialsLength)
		for iB, elB := range B {
			if elA == elB {
