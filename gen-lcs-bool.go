...
// BoolLCS returns the longest common subsequence of two given bool slices,
...
func BoolLCS(A, B []bool) (int, []bool) {
	if len(A) == 0 || len(B) == 0 {
		return 0, []bool{}
	}

	startIndex := findStartIndexBool(A, B)
	endIndexA, endIndexB := findEndIndicesBool(A, B, startIndex)
	...
	partialLCS := doLCSBool(A[startIndex:endIndexA], B[startIndex:endIndexB])

	lcs := make([]bool, startIndex)
...
func findStartIndexBool(A, B []bool) int {
	start, i, j := 0, 0, 0
...
func findEndIndicesBool(A, B []bool, startIndex int) (int, int) {
...
func doLCSBool(A, B []bool) []bool {

	partialsLength := len(B) + 1
	partials := make([][]bool, partialsLength)

	for _, elA := range A {
		newPartials := make([][]bool, partialsLength)
		for iB, elB := range B {
			if elA == elB {
