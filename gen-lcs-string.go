...
// StringLCS returns the longest common subsequence of two given string slices,
...
func StringLCS(A, B []string) (int, []string) {
	if len(A) == 0 || len(B) == 0 {
		return 0, []string{}
	}

	startIndex := findStartIndexString(A, B)
	endIndexA, endIndexB := findEndIndicesString(A, B, startIndex)
	...
	partialLCS := doLCSString(A[startIndex:endIndexA], B[startIndex:endIndexB])
...
func findStartIndexString(A, B []string) int {
	start, i, j := 0, 0, 0
...
func findEndIndicesString(A, B []string, startIndex int) (int, int) {
...
func doLCSString(A, B []string) []string {
